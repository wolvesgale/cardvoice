      ...
      <div className="sticky top-0 z-10 bg-neutral-950 border-b border-neutral-900 px-4 py-3">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-2">
            {profile?.plan === 'team' && (
              <Link href="/team" className="text-xs bg-emerald-900 text-emerald-400 px-3 py-1.5 rounded-lg">👥 チーム</Link>
            )}
            <Link href="/settings" className="text-xs bg-neutral-800 px-3 py-1.5 rounded-lg text-neutral-300">設定</Link>
          </div>
        </div>
        <form>
          <input name="q" defaultValue={searchParams.q} placeholder="名前・会社名・メモで検索..."
            className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-2 text-sm text-white placeholder-neutral-600 focus:outline-none focus:border-emerald-600" />
        </form>
      </div>
            ...
            <div className="bg-neutral-900 rounded-2xl p-4 border border-neutral-800 hover:border-neutral-700 transition-colors">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  ...
                  {card.follow_up_at && <p className="text-xs text-emerald-500 mt-0.5">📅 {new Date(card.follow_up_at).toLocaleDateString('ja-JP')}</p>}
                </div>
              </div>
